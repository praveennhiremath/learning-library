...
Go to SQL developer and execute below queries.
   
1. Run the below SQL to fetch TABLE1 and TABLE2 names from the edges and combine output and remove the duplicates. 

   	```<copy>
	SELECT DISTINCT TABLE1 FROM EDGES WHERE TABLE1 IN (TABLES_FROM_SELECTED_CLUSTERS_IN_SQL_FORMAT) 
	OR TABLE2 IN (TABLES_FROM_SELECTED_CLUSTERS_IN_SQL_FORMAT);
	</copy>```
	
   	```<copy>
	SELECT DISTINCT TABLE2 FROM EDGES WHERE TABLE1 IN (TABLES_FROM_SELECTED_CLUSTERS_IN_SQL_FORMAT)
	OR TABLE2 IN (TABLES_FROM_SELECTED_CLUSTERS_IN_SQL_FORMAT);
	</copy>```
	
	Merge the result of above 2 queries and have the tables of connected nodes as well.

	The below are the result tables names.We call it as TABLES_INCLUDING_CONNECTED_NODES.
	```<copy> 
...
1. Create new NODES table of the graph by running following SQL. 

	```<copy>
   CREATE TABLE NODES_259_NEW AS SELECT * FROM NODES WHERE TABLE_NAME IN (TABLES_INCLUDING_CONNECTED_NODES)
	</copy>```
2. Create new EDGE table of the graph by running following SQL.    
	```<copy>
...
3. Adding the constraints for the newly extracted data by running following SQL.
