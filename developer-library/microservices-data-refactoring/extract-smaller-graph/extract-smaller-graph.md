...
1. Replace #{TABLES-FROM-SELECTED-CLUSTERS-IN-SQL-FORMAT} with values copied from Task 1 -> Step 4 and run the below SQL to fetch from columns TABLE1 and TABLE2 names from the EDGES table and combine the output and remove the duplicates.
   	...
   	```
	<copy>
	SELECT DISTINCT TABLE1 FROM EDGES WHERE TABLE1 IN (#{TABLES-FROM-SELECTED-CLUSTERS-IN-SQL-FORMAT}) 
	OR TABLE2 IN (#{TABLES-FROM-SELECTED-CLUSTERS-IN-SQL-FORMAT});
	</copy>
	```
 
   	```
	<copy>
	SELECT DISTINCT TABLE2 FROM EDGES WHERE TABLE1 IN (#{TABLES-FROM-SELECTED-CLUSTERS-IN-SQL-FORMAT})
	OR TABLE2 IN (#{TABLES-FROM-SELECTED-CLUSTERS-IN-SQL-FORMAT});
	</copy>
	```
